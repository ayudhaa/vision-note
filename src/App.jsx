...
import { Mic, MicOff, Trash2, Sparkles, Clock, CheckCircle2, Volume2, Play } from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    if (SpeechRecognition) {
      ...
      Notiflix.Report.failure("Browser Tidak Didukung", "Browser kamu tidak mendukung Speech Recognition. Gunakan Chrome atau Edge terbaru.", "Oke");
    }
  ...
  const saveNote = () => {
    ...
    setTranscript("");
    Notiflix.Notify.success("Catatan berhasil disimpan!");
  };
  ...
  const playNote = (text) => {
    ...
    Notiflix.Notify.info("Memutar audio...");
  };
        ...
        <div className="bg-white rounded-[2.5rem] p-8 md:p-12 shadow-xl shadow-slate-200/50 border border-slate-100 mb-12 relative overflow-hidden text-center">
          ...
          <div className="relative z-10 flex flex-col items-center">
            <div className="relative mb-8">
              {isListening && (
                <div className="absolute inset-0 bg-orange-200 rounded-full animate-ping opacity-20 scale-150"></div>
              )}
              <button 
            ...
            <div className="w-full min-h-[100px] flex items-center justify-center px-4">
              {transcript ? (
                <p className="text-xl font-semibold text-slate-700 leading-tight italic">
                  "{transcript}"
                </p>
              ) : (
            ...
            {transcript && !isListening && (
              <div className="flex gap-4 mt-8">
                <button 
                  onClick={saveNote}
                  className="bg-slate-900 hover:bg-black text-white px-10 py-4 rounded-2xl font-bold text-xs uppercase tracking-[0.2em] shadow-lg transition-all"
                >
                  Simpan Catatan
                </button>
              </div>
